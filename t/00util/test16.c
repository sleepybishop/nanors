...
int test_codec_run(int K, int N, int T, int erasures_count, int seed)
{
    ...
    int decode_res = reed_solomon16_decode(rs, buf, marks, K + N, T);
    if (decode_res < 0)
        printf("reed_solomon16_decode returned %d\n", decode_res);
...
static void run_api_safety_tests(void)
{
    ...
    rs_t *rs1 = reed_solomon16_new(65536, 10);
    assert(rs1 == NULL);
    rs_t *rs2 = reed_solomon16_new(0, 5);
    assert(rs2 == NULL);
...
int main(int argc, char *argv[])
{
    ...
    printf("=== RUNNING RS16 CODEC TEST BATTERY ===\n");
    printf("Seed: %d\n", seed);
